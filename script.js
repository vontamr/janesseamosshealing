// ================================================
// Janesse's Seamoss Creation - Full Cart System
// Works with shop.html AND build.html
// ================================================
    ...
    html += `
      <div class="flex gap-4 py-6 border-b">
        <div class="flex-1">
          ...
          <p class="text-sm text-gray-500">$${item.price} × ${item.qty || 1}</p>
        </div>
        <div class="text-right">
          <p class="font-semibold">$${(item.price * (item.qty || 1)).toFixed(2)}</p>
          <div class="flex items-center gap-3 mt-3 justify-end">
            ...
            <span class="w-6 text-center">${item.qty || 1}</span>
            <button onclick="updateQuantity('${item.id}', 1)" class="w-7 h-7 border border-purple/30 rounded-xl hover:bg-purple/5">+</button>
          </div>
...
async function proceedToCheckout() {
  ...
  try {
    const response = await fetch('https://janesse-seamoss-backend.onrender.com/create-checkout-session', {
      ...
      body: JSON.stringify({
        cart: cart,
        success_url: 'https://vontamr.github.io/janesseamosshealing/shop.html?success=true',
        cancel_url: 'https://vontamr.github.io/janesseamosshealing/cancel.html'
      })
    });
    ...
    alert("⚠️ Unable to connect to payment. Please try again or contact us for help.");
  }
}
...
window.addEventListener('load', () => {
  ...
  if (urlParams.get('success') === 'true') {
    showToast("✅ Payment successful! Thank you for supporting your wellness journey 🌿");
    cart = [];
    saveCart();
    updateCartUI();
  }
